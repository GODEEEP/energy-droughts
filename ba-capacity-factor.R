...
solar_fn_y2 = sprintf('data/tgw-gen/solar/rcp85hotter/solar_gen_cf_%s.csv',y2)
wind_fn_y1 = sprintf('data/tgw-gen/wind/historical/wind_gen_cf_%s.csv',y1)
wind_fn_y2 = sprintf('data/tgw-gen/wind/rcp85hotter/wind_gen_cf_%s.csv',y2)

solar_list = wind_list = load_list = list()

solar_wide = bind_rows(read_csv(solar_fn_y1, show=FALSE, progress=FALSE),
                       read_csv(solar_fn_y2, show=FALSE, progress=FALSE))
wind_wide = bind_rows(read_csv(wind_fn_y1, show=FALSE, progress=FALSE),
...
bas = plant_counts |> pull(ba) |> unique() |> sort()
...
scaling_factor = solar_wind_ba_ave |> 
  ...
  mutate(year = year(datetime_utc),
         datetime_utc = case_when(year(datetime_utc) == y1 ~ datetime_utc,
...
sf_hourly = scaling_factor |> 
  mutate(wind_sf=ifelse(`wind_cf-2017`==0,1,`wind_cf-2057`/`wind_cf-2017`),
         solar_sf=ifelse(`solar_cf-2017`==0,1,`solar_cf-2057`/`solar_cf-2017`)) |>
  mutate(wind_sf = ifelse(wind_sf > 3, NA, wind_sf),
...
sf_heatwave = sf_hourly |> 
  ...
  group_by(ba) |>
  summarise(wind_sf = mean(wind_sf, na.rm=T),
            solar_sf = mean(solar_sf, na.rm=T))

sf_monthly = 
  sf_hourly |>
  group_by(ba,month) |> 
  summarise(`Wind`=median(wind_sf,na.rm=T),
...
ratios_mean_ba = ratios_monthly_mean_ba |> 
  rename(ba=BA) |>
  group_by(ba) |> 
  summarise(load=mean(Mean_Load_Ratio),
