...
import::from(dplyr, filter, pull, lag, arrange, left_join, group_by, mutate, bind_rows, summarise, case_when)
...
configs = bind_rows(wind_config |> mutate(type='wind'), 
  ...
  mutate(ba = case_when(ba == 'ERCO' ~ "ERCOT",
                        ...
                        ba == 'NYIS' ~ "NYISO",
                        .default = ba)) 
ba_centroids = configs |> group_by(ba) |> summarise(lon=mean(lon),lat=mean(lat))
# 
period_names = periods |> names() |> strsplit('_') |> sapply('[',2)
...
read_droughts <- function(prefix){
  ...
  for(i in 1:length(periods)){
    ...
    droughts_list[[i]] = read_csv(sprintf('data/droughts/%s_%s.csv', prefix, period_name_noindex), 
      ...
      mutate(period = period_name_noindex,
...
ws_lws_droughts = bind_rows(ws_droughts |> mutate(type='Wind and Solar Droughts'),
                            lws_droughts |> mutate(type='Load, Wind, and Solar Droughts')) |>
  mutate(type=factor(type,types),
         severity=ifelse(type=='Wind and Solar Droughts', severity_ws, severity_lws))
...
drought_stats = function(d, sev_type='ws'){
  d |> group_by(ba,period,lon,lat) |> 
    summarise(median_dur = median(run_length_days),
...
ws_max_dur_sum = ws_drought_stats |> filter(stat=='max_dur') |> group_by(ba) |> summarise(max=sum(value))
ws_max_dur_order = ws_max_dur_sum[order(ws_max_dur_sum$max),]$ba

lws_drought_stats = drought_stats(lws_droughts, sev_type='lws') 
lws_max_dur_sum = lws_drought_stats |> filter(stat=='max_dur') |> group_by(ba) |> summarise(max=sum(value))
lws_max_dur_order = lws_max_dur_sum[order(lws_max_dur_sum$max),]$ba

types = c('Wind and Solar Droughts','Load, Wind, and Solar Droughts')
ws_lws_drought_stats = bind_rows(ws_drought_stats |> mutate(type='Wind and Solar Droughts'),
                                 lws_drought_stats |> mutate(type='Load, Wind, and Solar Droughts')) |>
  mutate(type=factor(type,types))
...
##########################
p_ws_drought_max = 
  ws_drought_stats |> filter(stat=='max_dur') |> 
  ggplot() + 
  theme_minimal() +
...
p_lws_drought_max = 
  lws_drought_stats |> filter(stat=='max_dur') |> 
  ggplot() + 
  theme_minimal() +
...
p_ws_frequency = 
  ggplot(ws_droughts) + 
  geom_bar(aes(period, ..count../n_years, fill=ba), stat='count', position='dodge') +
...
max_freq_at_dur <- function(x, binsize=2){
  ...
  line = x |> mutate(dur_bin = cut(max_dur,breaks),
...
lines = bind_rows(
  max_freq_at_dur(ws_lws_drought_stats_wide |> 
                    filter(type=='Load, Wind, and Solar Droughts')),
  max_freq_at_dur(ws_lws_drought_stats_wide |> 
                    filter(type=='Wind and Solar Droughts'))
)

a1 = 2; b1 = 18; c1 = -0.6; d1 = 2; cx1 = seq(a1+.55,15, by=.1); cy1 = a1+b1*exp(c1*(cx1-d1))
a2 = 1.5; b2 = 15; c2 = -0.7; d2 = 1.5; cx2 = seq(a2+.75,11, by=.1); cy2 = a2+b2*exp(c2*(cx2-d2))
ub_curves = bind_rows(data.frame(x=cx1, y=cy1, type='Wind and Solar Droughts'),
                      data.frame(x=cx2, y=cy2, type='Load, Wind, and Solar Droughts')) |>
  mutate(type=factor(type,types))
...
max_dur_freq_mean = ws_lws_drought_stats_wide |> 
  ...
  summarise(max_freq = max(frequency), max_dur = mean_dur[which.max(frequency)])
...
a = 2
b = 0
ba_labels = ba_labels |> mutate(lat = lat + c(BPAT=a, CISO=a+1, ERCOT=a+1, IPCO=a, ISONE=a, MISO=a, NWMT=a, NYISO=a,
                                              PACE=a+1, PACW=-a, PJM=a, PNM=-a-1, PSCO=-a+.2, SWPP=a+1, WACM=a+1),
                                lon = lon + c(BPAT=b, CISO=b, ERCOT=b, IPCO=b, ISONE=b+3, MISO=b, NWMT=b, NYISO=b,
                                              PACE=b, PACW=b, PJM=b-2.8, PNM=b, PSCO=b-3.4, SWPP=b, WACM=b))
p_ws_freq_dur_1d = ws_lws_drought_stats_wide |> 
  filter(period=='1-day' & type =='Wind and Solar Droughts') |> 
...
ggsave('plots-idf/map_ws_freq_dur_1d.png', p_ws_freq_dur_1d, width=8, height=4.2, dpi=600)
 ...
 p_ws_freq_dur_1h = ws_lws_drought_stats_wide |> 
  ...
  #scale_color_viridis_c(option='H')+
  scale_size_continuous('Events\nPer\nYear', range=c(1,15), breaks=c(5,9,13), limits=c(4,15))+
  #scale_color_stepsn('Drought\nDuration\n(Days)', colors=brewer.pal(6,'YlOrRd'), limits=c(0,5), n.breaks=6)+
  # scale_fill_viridis(
  #   option="B",
  #   trans = "log", 
...
p_ws_freq_dur_1h_1d
ggsave('plots-idf/map_ws_freq_dur_1h_1d.png', width=17, height=4, dpi=600)


p_ws_freq_dur_multiple = ws_lws_drought_stats_wide |> 
  filter(period%in%c('1-hour','1-day') & type =='Wind and Solar Droughts') |> 
  #mutate(dur_binned = cut(p90_dur, breaks = 0:6)) |>
  ggplot(aes(lon,lat)) + 
  geom_polygon(aes(long, lat, group=group), fill='#E3DEBF',data=world)+
  geom_polygon(aes(long, lat, group=group), color='white', fill=grey(.6),data=states, size=.2)+
...
p_lws_freq_dur = ws_lws_drought_stats_wide |> 
  filter(period=='1-day' & type =='Load, Wind, and Solar Droughts') |> 
  ggplot(aes(lon,lat)) + 
...
configs |> 
  ...
  filter(ba %in% bas) |> left_join(read_csv('data/ba-names.csv'),by='ba') |>
